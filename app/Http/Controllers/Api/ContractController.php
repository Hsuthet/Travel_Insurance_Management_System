...
class ContractController extends Controller
{
    ...
    public function apply(Request $request)
    {
        ...
        if ($request->plan_id == 3) {
            ...
            $rules['beneficiary_info.passport'] = 'nullable|string|max:20';
        }
        ...
        try {
            ...
            // Fix for Customer::where
            $customer = Customer::query()
                ->where('nrc', '=', $nrcFormatted)
                ->first();

            // 4. Duplicate Check
            if ($customer) {
                // Fix for Contract::where
                $duplicateContract = Contract::query()
                    ->where('customer_id', '=', $customer->customer_id)
                    ->where('destination', '=', $request->destination)
                    ->where('start_date', '=', $request->start_date)
                    ->where('end_date', '=', $request->end_date)
                    ->where(function ($query) {
                        $query->whereIn('status', ['pending', 'wait_pay', 'approved']);
                    })
                    ->first();
    ...
    private function generateContractId()
    {
        ...
        // Fix for Contract::where LIKE
        $latestContract = Contract::query()
            ->where('contract_id', 'LIKE', $prefix.'%')
